...
export default function Applications() {
	...
	useEffect(() => {
		if (isError) {
			...
			if (error?.data?.parol) toast.error('Parolingizni xato kiritdingiz')
		}
	}, [isError, error])
	...
	return (
		<div className='w-full space-y-4'>
				...
				<Card className='w-full border-border/50 shadow-lg bg-card/95 backdrop-blur-sm'>
					...
					<CardContent className='px-6 py-5'>
						<form className='space-y-6' onSubmit={handleSubmit}>
							...
							{/* Per-target extra comments */}
							{form.targets.map((item, index) => (
								<div
									key={item.tuzilma}
									className='space-y-2 p-3 rounded-lg bg-muted/30 border border-border/40'
								>
									...
									<Textarea
										...
										placeholder='Tuzilmaga maxsus xabar...'
										className='resize-none h-16 text-sm bg-card border-border/50'
										onChange={e => {
											setForm(prev => {
							...
							<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
								<div className='space-y-2'>
									<Label className='text-sm font-semibold'>Turi</Label>
									<Select
										value={form.turi}
										onValueChange={val => setForm({ ...form, turi: val })}
									>
										<SelectTrigger className='h-9 text-sm bg-card border-border/60'>
									...
									<div className='space-y-2'>
										...
										<Popover>
											<PopoverTrigger asChild>
												<Button
													variant='outline'
													className='w-full justify-start text-left font-normal gap-2 bg-card border-border/60 h-9 text-sm'
												>
							...
							<div className='space-y-2'>
								...
								<div className='flex flex-wrap gap-2 items-start'>
									<label className='flex flex-col items-center justify-center w-20 h-20 border-2 border-dashed border-primary/30 rounded-xl cursor-pointer hover:bg-primary/5 hover:border-primary/60 transition-all flex-shrink-0'>
										<Plus className='w-5 h-5 text-primary/60 mb-0.5' />
									...
									{form.photos.map((photo, index) => (
										<div
											key={index}
											className='relative w-20 h-20 rounded-xl bg-muted border border-border/50 shadow-sm flex-shrink-0 overflow-hidden group'
										>
											<img
			...
			>
				<DialogContent className='sm:max-w-sm border-border/60 bg-card shadow-2xl'>
					<DialogHeader className='pb-2'>
						...
						<p className='text-xs text-muted-foreground'>
							Arizani jo'natish uchun parolingizni kiriting
						</p>
					</DialogHeader>
					<div className='space-y-4 pt-1'>
						<div className='relative'>
							<Input
								...
								className='pr-10 text-sm bg-muted/40 border-border/60 h-10'
								value={form.parol}
						...
						<div className='flex gap-2'>
							<Button
								type='button'
								variant='outline'
								className='flex-1 text-sm h-9 border-border/60 bg-transparent'
				...
				<Table>
					<TableHeader>
						<TableRow className='bg-muted/40 hover:bg-muted/40 border-border/40'>
							<TableHead className='w-[18%] text-xs font-semibold text-muted-foreground uppercase tracking-wide py-3'>
								Kim tomonidan
							</TableHead>
							...
							<TableHead className='w-[17%] text-xs font-semibold text-muted-foreground uppercase tracking-wide'>
								Izoh
							</TableHead>
